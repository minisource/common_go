...
func (j *JWTManager) PreparePayload() map[string]interface{} {
	scopesJSON, err := json.Marshal(j.scopes)
	if err != nil {
		fmt.Errorf("Failed to serialize scopes: %v", err)
	}

	return map[string]interface{}{
		"client_id":     j.clientID,
		"client_secret": j.clientSecret,
		"scopes":        string(scopesJSON), // JSON-serialized array
	}
}
